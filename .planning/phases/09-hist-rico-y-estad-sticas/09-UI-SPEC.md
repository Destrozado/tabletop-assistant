...
| Outcome dialog retained warning (D-02; text corrected en la ronda 6, plan 09-30; promesa de reintento retirada en la ronda 7, plan 09-32) | `buildEndGameBody` (`app/composables/useGameEndCopy.ts`), citado literalmente: `El progreso guardado de esta partida ({resumen}) se borrará y volveréis a la pantalla de inicio. Si el registro en el histórico falla, el progreso no se borrará y la app os avisará.` — la ronda 7 (`09-VERIFICATION.md`) encontró que la versión anterior prometía «la app conservará el progreso para que podáis reintentarlo» ANTES de conocer el estado del dispositivo, promesa falsa en 2 de los 4 estados posibles (`absent`: el aviso posterior dice que no hay nada que reintentar; `stale`: el aviso posterior deliberadamente no ordena el reintento). Lo que queda es exactamente lo que `preserveProgress = !historyRecorded` garantiza en las cuatro salidas: que el progreso no se borra si el registro falla, sea cual sea el estado del dispositivo — nada sobre poder reintentar. D-02's *intent* (warn before ending the game) is kept; the *fact it asserts* is corrected again. |
...
| Save-result notice — failure, `failure-stale` (new in ronda 6, plan 09-28; reescrita en la ronda 7, plan 09-32) | Heading: `⚠ No se pudo guardar la partida` · Body, citado literalmente de `NOTICE_BODY` en `app/composables/useHistorySavedNotice.ts`: `La partida no se ha registrado. En el dispositivo queda guardada una partida de este juego, pero no coincide con el punto en el que habéis terminado: la app no puede saber si es esta misma partida o es otra. Registrarla desde ahí podría guardar en el histórico datos que no son los de esta partida. Suele deberse al modo privado del navegador o a la memoria llena.` — la versión de la ronda 6 (citada arriba hasta este lote) afirmaba dos hechos que `esLaMismaPartida` no establece: anterioridad temporal («una versión anterior» — `updatedAt` está excluido de la comparación por escrito, así que no hay ningún orden temporal comprobado) y diferencia de ronda («no la ronda en la que habéis terminado» — literalmente falso en el escenario canónico de `avisoTrasRegistroFallido.test.ts` test 5, donde la única diferencia real está en `runtimeId` y las dos rondas son iguales). La versión nueva solo afirma que hay algo guardado y que no coincide con el punto de fin de partida, sin identificar qué campo difiere ni ordenar ningún reintento. |
| Resume prompt — aviso de progreso que no coincide (ronda 7, plan 09-33) | `mismatchWarning` en `ResumePrompt.vue`, citado literalmente de `PROGRESS_MISMATCH_WARNING` en `app/composables/useProgressMismatchMark.ts`: `Aviso: al terminar la última partida de este juego, la app no pudo registrarla y comprobó que lo que había guardado no era el punto en el que habíais terminado. Puede que esta partida no sea la que terminasteis: si la continuáis y la registráis, el histórico podría quedar con datos que no son los de aquella partida.` — se muestra únicamente cuando el último cierre de partida de ese juego encontró discrepancia (`historyRecorded === false && stored === 'stale'`) y el progreso que dejó sigue ahí; la marca es estado de módulo en memoria (nunca `localStorage`), así que una recarga completa del navegador la pierde y el aviso deja de mostrarse sin que eso signifique que la discrepancia desapareció. |
...
*Note (ronda 7, planes 09-32..09-35): the three rows this lote touches — outcome dialog retained warning, `failure-stale`, and the new resume-prompt mismatch row — cite the literal code text as it stands after 09-32..09-35, not the plan-time draft nor the ronda 6 text they replace. See `09-VERIFICATION.md` ronda 7 for the three findings this lote closes and `09-36-SUMMARY.md` for the byte-for-byte cross-check against the `.ts` source.*
